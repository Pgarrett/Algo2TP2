...
\subsubsection{Representación de Juego}
	...
	\begin{Estructura}{Juego}[estr]
		\begin{Tupla}[estr]
			\tupItem{pokemones}{diccString(Pokemon, ListaPorTipo: ListaEnlazada(itConj(infoPokemon)))}%
...
\subsubsection{Funci\'on de Abstracci\'on}	
	Abs(e): estre - $>$ Juego {Rep(e)} 
 pGo: Juego tq e.mapa = mapa(pGo) $\land$ e.jugadores = jugadores(pGo) $\yluego$ \\
 ($\forall j : jugador$) j $\in$ e.jugadores $\impluego$ 
 \\ j.sanciones = sanciones(j, pGo) ((j $\in$ expulsados(pGo) $\land$ j.sanciones >= 10)\\
 $\oluego$ (j.pokesCapturados = pokemones(j,pGo) $\land$ j.estaConectado = estaConectad(j,pGo) \\
 $\land$ j.estaConectado $\impluego$  j.pos = posicion(j,pGo))) $\land$ \\
 ($\forall p : pokemon$) p $\in$ c.pokemones $\impluego$ ($\forall j : Jugador$) \\
 j $\in$ e.jugadores $\yluego$ p $\in$ pokemones(j,pGo) o [($\forall c : coord$)\\
 c $\in$ e.mapa.coordenadas $\yluego$ p = pokemonEnPos(c,pGo) $\land$ cantMovParaCap(c,pGo)\\
 p.contador]
\end{Representacion}
...
\begin{algorithm}[H]{\textbf{iAgregarJugador}(\Inout{j}{estr}) $\to$ res : Jugador}
	\begin{algorithmic}
		...
		\State infoJ $\gets$ $<$itConjIds, false, 0, Vacio(), CrearIt(Vacio())$>$ \Comment O(1) {El jugador es creado vacio, sin sanciones y sin pokemones atrapados}
...
\begin{algorithm}[H]{\textbf{iAgregarJugadorAPos}(\Inout{d}{DiccMat(coor, puntero(conjLineal(Jugador)))}, \Inout{j}{infoJugador}, \In{c}{coor})} 
	\begin{algorithmic}
      ...
      \State j.posicion $\gets$ AgregarRapido(jugsEnPos, j.id) \Comment O(1)
...
\begin{algorithm}[H]{\textbf{iDesconectarse}(\Inout{j}{estr}, \In{id}{Jugador})} 
    ...
    \State EliminarSiguiente(infoJ.posicion) \Comment O(1)
    \State infoJ.posicion $\gets$ CrearIt(Vacio())	\Comment O(1)
		\medskip
		\Statex \underline{Complejidad:} $O(log(EC))$
