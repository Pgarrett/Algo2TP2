...
$\textbf{Pre}$ $\equiv$ $\{$ d $=$ d_0 $\land$ longitud(c) $<$ ancho(d) $\land$ latitud(c) $<$ largo(d) $\}$
  \begin{algorithmic}
...
\begin{algorithm}[H]{\textbf{iMoverse}(\Inout{j}{estr}, \In{id}{Jugador}, \In{c}{coor})}{}
    ...
    \If {iHayPokemonCercano(c, j)}	\Comment O(1) \textbf{Se movio en el rango}
       \State iActualizarMenos(c, j)	\Comment O(PS)
    \Else \Comment \textbf{Salio del rango}
    ...
    \EndIf
\Else
    \If {iHayPokemonCercano(c, j)}	\Comment O(1) \textbf{Entro a un rango}
      \State iActualizarMenos(c, j)	\Comment O(PS)
    ...
    \Else \Comment \textbf{Se movio por afuera de los rangos}
    		\State iActualizarTodos(j)	\Comment O(PS)
    \EndIf
	\EndIf
	\If {debeSancionarse?(id, c, j)}
  \State infoJ.sanciones $\gets$ infoJ.sanciones + 1	\Comment O(1)
		\If {infoJ.sanciones $<$ 5}	\Comment O(1)
			\State EliminarSiguiente(infoJ.itPosicion) \Comment O(1)
			\State AgregarJugadorEnPos(j.posicionesJugadores, infoJ, c) \Comment O(1)
		\Else
