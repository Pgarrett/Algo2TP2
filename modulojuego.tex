...
\begin{algorithm}[H]{\textbf{iDebeSancionarse}(\In{e}{jugador}, \In{c}{coor}, \In{j}{juego}) $\to$ $res$ : $Mapa$} 
\\
...
\begin{algorithm}[H]{\textbf{iPuedoAgregarPokemon}(\In{j}{juego}, \In{c}{coor}) $\to$ $res$ : $bool$} 
 ...
 \begin{algorithm}[H]{\textbf{HayPokemonCercano}(\In{j}{juego}, \In{c}{coor}) $\to$ $res$ : $bool$} 
	\begin{algorithmic}
		...
		\State i $\gets$ 0 \Comment $O(1)$
		\State latC $\gets$ Latitud(c)	\Comment $O(1)$
		\If {latC $>$ 5}	\Comment $O(1)$
			\State i $\gets$ latC - 5	\Comment $O(1)$
		\Else
			\If {latC $>$ 4}	\Comment $O(1)$
				\State i $\gets$ latC - 4	\Comment $O(1)$
			\Else
				\If {latC $>$ 3}	\Comment $O(1)$
		...
		\State j $\gets$ 0 \Comment $O(1)$
		\State longC $\gets$ Longitud(c)	\Comment $O(1)$
		\If {longC $>$ 5}	\Comment $O(1)$
			\State j $\gets$ longC - 5	\Comment $O(1)$
		\Else
			\If {longC $>$ 4}	\Comment $O(1)$
				\State j $\gets$ longC - 4	\Comment $O(1)$
			\Else
				\If {longC $>$ 3}	\Comment $O(1)$
