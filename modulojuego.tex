...
%			\tupItem{todosLosPokemones}{conjLineal(infoPokemon)}
			...
			\tupItem{posicionesPokemons}{DiccMat(coor, $<$Pokemon, itConj(infoPokemon)$>$)} %% itConj es un iterador al conjunto de todosLosPokemones
	...
	\end{Estructura}
      ...
      \tupItem{posicion}{itConj(Jugador)}
		\end{Tupla}
...
\begin{algorithm}[H]{\textbf{iAgregarJugador}(\Inout{j}{estr}) $\to$ res : Jugador}
	\begin{algorithmic}
		...
		\State infoJ $\gets$ $<$id, false, 0, Vacio(), CrearIt(Vacio())$>$ \Comment O(1) {El jugador es creado vacio, sin sanciones y sin pokemones atrapados}
...
\begin{algorithm}[H]{\textbf{iConectarse}(\Inout{j}{estr}, \In{e}{Jugador}, \In{c}{coor})}
    ...
    \If {$\lnot$ Definido?(j.posicionesJugadores, c)}	\Comment O(1)
        \State Definir(j.posicionesJugadores, c, $\&Vacio()$) \Comment O(1)
    \EndIf
    \State jugsEnPos $\gets$ *(Significado(j.posicionesJugadores, c)) \Comment O(1)
    \State Siguiente($j.jugadoresPorID[e]_{1}$).posicion $\gets$ AgregarRapido(jugsEnPos, e) \Comment O(1) 
...
\begin{algorithm}[H]{\textbf{iDesconectarse}(\Inout{j}{estr}, \In{id}{Jugador})} 
	\begin{algorithmic}
		...
		\EndIf
        \State Siguiente($tupJug_{1}$).estaConectado $\gets$ false	\Comment O(1)
	  
        \State EliminarSiguiente(Siguiente($tupJug_{1}$).posicion) \Comment O(1)
        \State Siguiente($tupJug_{1}$).posicion $\gets$ CrearIt(Vacio())	\Comment O(1)
...
\begin{algorithm}[H]{\textbf{Moverse}(\Inout{j}{estr}, \In{id}{Jugador}, \In{c}{coor})}{}
	\begin{algorithmic}
