...
\begin{enumerate}
	\item Raiz es la raiz del arbol con invariante de trie y es un arreglo de 256 posiciones.
	\item Cada uno de los elementos de la lista tiene que ser un puntero a un Nodo del trie.
	\item Nodo es una tupla que contiene un arreglo de 256 posiciones con un puntero a otro Nodo en cada posicion
...
\tadAlinearFunciones{iteradorValido}{puntero(Nodo)/nodo}
...
\tadAxioma{nodoValido($nodo$)}{
	longitud(*nodo.arbolTrie) == 256 $\yluego$
		\\
...
\begin{algorithm}[H]{\textbf{iDefinir}(\Inout{d}{estr}, \In{c}{string}, \In{s}{$\alpha$})}
	\begin{algorithmic}
		...
		\EndIf
		\State $puntero(Nodo): arr \gets d.raiz[letra]$ \Comment $O(1)$
		\While{$i < longitud(c)$} \Comment $O(|c|)$
			\State $i \gets i + 1$ \Comment $O(1)$
			\State $letra \gets ord(c[i])$ \Comment $O(1)$
...
\begin{algorithm}[H]{\textbf{iEliminar}(\Inout{d}{estr}, \In{c}{string})}
	\begin{algorithmic}
		...
		\Else
			...
			\While{$i < longitud(c)  \land \neg tieneHermanosEInfo(*tope(pil))$} \Comment $O(|c|)$
