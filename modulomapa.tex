...
\begin{Interfaz}
  ...
  \InterfazFuncion{AgregarCoordenada}{\Inout{m}{map}, \In{c}{Coordenada}}{itConj(Coordenada)}
  [$m \igobs m_0$]
  ...
  \InterfazFuncion{Coordenadas}{\In{m}{map}}{itConj(Coordenada)}
  [true]
  {$res \igobs coordenadas(m)$}
  [$O(1)$]
  [Devuelve un iterador al conjunto de coordenadas del mapa]
  
  \InterfazFuncion{PosExistente}{\In{c}{Coordenada}, \In{m}{map}}{Bool}
  [true]
  {$res \igobs posExistente(c, m)$}
  [$\Theta\left(\displaystyle\sum_{c' \in coordendas(m)}equal(c,c')\right)$]
  [Devuelve verdadero si la coordenada esta en el conjunto de coordenadas del mapa]
  
  \InterfazFuncion{HayCamino}{\In{c1}{Coordenada}, \In{c2}{Coordenada}, \In{m}{map}}{Bool}
...
\subsubsection{Representación de Mapa}
	\begin{Estructura}{Mapa}[estr]
		\begin{Tupla}[estr]
			\tupItem{coordenadas}{ConjLineal(Coordenada)}%
			\tupItem{ancho}{Nat}
			\tupItem{secciones}{DiccMat(Coordenada, Nat)}
		\end{Tupla}
	\end{Estructura}
...
\subsubsection{Invariante de Representación}
	...
	\end{enumerate}
	\Rep[estr][e]{(e.ancho = Max((coordenadas)$_{2}$) $\land$ ($\forall c: Coordenada$) c $\in$ e.coordenadas $\impluego$ def?(c,e.secciones)}
...
\begin{algorithm}[H]{\textbf{iAgregarCoordenada}(\Inout {m}{estr}, \In {c}{Coordenada}) $\to$ $res$ : itConj(Coordenada)}
    	\begin{algorithmic}[1]
...
\begin{algorithm}[H]{\textbf{iDefinirSeccion}(\Inout {m}{estr}, \In {c}{Coordenada}, \In {i}{Nat})}
    	\begin{algorithmic}[1]
...
\begin{algorithm}[H]{\textbf{iCoordenadas}(\In {m}{estr}) $\to$ $res$ : itConj(Coordenada)}
    	\begin{algorithmic}[1]
...
\begin{algorithm}[H]{\textbf{iPosExistente}(\In{c}{Coordenada}, \In{m}{estr}) $\to$ $res$ : Bool}
    	\begin{algorithmic}[1]
...
\begin{algorithm}[H]{\textbf{iHayCamino}(\In{c1}{Coordenada}, \In{c2}{Coordenada}, \In{m}{estr}) $\to$ $res$ : Bool}
    	\begin{algorithmic}[1]
