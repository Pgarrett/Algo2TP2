...
El módulo Diccionario Matriz provee un diccionario por posiciones en el que se puede definir, y consultar si hay un valor en una posicion en tiempo $O(copy(\sigma))$. Ademas, se puede borrar en tiempo lineal sobre las dimensiones de la matriz, y obtener un iterador a un conjunto lineal de claves.
...
%TODO
  ...
  [$\Theta(copy(s))$]
  [devuelve \texttt{true} si y sólo si $c$ tiene un valor en el diccMat.]
...
\begin{Representacion}
	\begin{Estructura}{Diccionario Matriz}[dicc]
		\begin{Tupla}[dicc]
			\tupItem{posiciones}{arreglo_dimensionable de <$bool$, $\sigma$>}
...
\begin{algorithm}[H]{\textbf{iDefinir}(\Inout {d}{diccMat}, \In {c}{coor}, \In {s}{$\sigma$})}
    	\begin{algorithmic}[1]
            ...
            \Statex \underline{Justificación:} $Definido?$ y $Aplanar$ tienen costo $\Theta(1)$, $AgregarRapido$ y $Definir$ tienen costo $\Theta(copy(s))$. Aplicando algebra de ordenes: $\Theta(1)$ + $\Theta(1)$ + $\Theta(copy(s))$  + $\Theta(copy(s))$ = $\Theta(copy(s))$
        \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]{\textbf{iDefinido?}(\In {d}{diccMat}, \In {c}{coor}) $\to$ $res$ : $bool$}
        ...
        \State $res \gets Definido?(d.posiciones, Aplanar(d, c)) $\land_L$ d.posiciones[Aplanar(d, c)]_0$ } \Comment{Si no esta definido o esta marcado como borrado, se devuelve que no esta definido} $\Theta(1)$
            ...
            \Statex \underline{Justificación:} $Aplanar$ tiene costo $\Theta(1)$, luego, como $Definido?$ y consular una posicion de un arreglo tienen costo $\Theta(1)$. Aplicando algebra de ordenes: $\Theta(1)$ + $\Theta(1)$ + $\Theta(1)$ = $\Theta(1)$
        \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]{\textbf{iAplanar}(\In {d}{diccMat}, \In {c}{coor}) $\to$ $res$ : $nat$}
    	\begin{algorithmic}[1]
			...
			\medskip
			\Statex \underline{Complejidad:} $\Theta(1)$
            \Statex \underline{Justificación:} Son operaciones matematicas de Nat
        \end{algorithmic}
\end{algorithm}
