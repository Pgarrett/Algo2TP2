...
%			\- \- \- \- donde \TipoVariable{encolado} es \TipoVariable{tupla}($prior$: nat, $elemCola$: itLista(Nodo))			
	...
	\Rep[estr][e]{
		...
		\\(e.proximo $\neq$ NULL) $\impluego$ (
		\\(e.proximo $\rightarrow$ izq $\neq$ NULL $\yluego$ ((e.proximo $\rightarrow$ prior $>$ (e.proximo $\rightarrow$ izq) $\rightarrow$ prior) $\wedge$ (e.proximo = (e.proximo $\rightarrow$ izq) $\rightarrow$ padre))) 
		\\(e.proximo $\rightarrow$ der $\neq$ NULL $\yluego$ ((e.proximo $\rightarrow$ prior $>$ (e.proximo $\rightarrow$ der) $\rightarrow$ prior) $\wedge$ (e.proximo = (e.proximo $\rightarrow$ der) $\rightarrow$ padre))) $\wedge$
		\\ Rep(SubArbolIzq(e)) $\wedge$ Rep(SubArbolDer(e)))
	}\mbox{}

\tadAlinearFunciones{SubArbolIzq}{c/estr}
\tadOperacion{SubArbolIzq}{c/est}{estr}{}
\tadOperacion{SubArbolDer}{c/est}{estr}{}

	\tadAxioma{SubArbolIzq($c$)}{
		estr(c.proximo $\rightarrow$ izq, c.tamaño$-$1)
	}

	\tadAxioma{SubArbolDer($c$)}{
		estr(c.proximo $\rightarrow$ der, c.tamaño$-$1)
	}
